...
static void rk817_charge_irq_delay_work(struct work_struct *work)
{
	...
	wake_lock_timeout(&charge->wake_lock,
			 3 * HZ);
    printk("rk817_charge_irq_delay_work wake_lock_timeout\n");
...
wake_lock_init(&charge->wake_lock, WAKE_LOCK_SUSPEND, "rk817_bat_lock");
