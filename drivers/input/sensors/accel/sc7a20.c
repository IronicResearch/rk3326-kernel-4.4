...
static struct workqueue_struct *sc7660_workqueue = NULL;//allen
...
struct SC7660_acc offset_yj;
static int calibrated;
struct i2c_client *sc7660_client_yj;
...
//add by quanzhiyi /20140310
static int SC7660_read_file(char *path, s16 *buf, int size)
{
    struct file *filp;
    loff_t len, offset;
    int ret=0;
...
//static void SelectSort(signed char array[], int length)
//{
//	int i, j, k;
//	signed char temp;
//	for(i = 0; i < length; i++)
...
unsigned char Read_Reg_yj(unsigned char reg)
{
	char buffer[3]={0};
	*buffer = reg;
	sensor_rx_data(sc7660_client_yj, buffer,1);
	return buffer[0];
}
...
//	int sum = 0;
//	char buffer[3] = {0};
//	signed char average = 0;	
//	signed char acc_buf[6];
//	signed char acc_array[6];
//        int index = 0;
//	int count = 0;
//        int ret   = 0;
//
...
//				ret = sensor_rx_data(sc7660_client_yj, buffer,1);				
//			
//				if( (buffer[0] & 0x08) != 0 )
//				{
//                                     	break;
...
//			if(reg == 0x29)     
//			{
//				acc_array[count] = acc_buf[1];			
//			}
//			else if(reg == 0x2b) 
//			{
//				acc_array[count] = acc_buf[3];
//			}
//			else if(reg == 0x2d) 
//			{
//				acc_array[count] = acc_buf[5];
//			}
//			else 
//				return 0;		
//	}
//	SelectSort(acc_array, length);
//	
//	for(i = 2; i < length - 2; i++)
//		sum += acc_array[i];
//	average = (signed char)(sum / (length -4));
//       
//	return average;
//	
//}
...
{
	int result;
	result = sensor_write_reg(sc7660_client_yj, addr, thedata);
}
...
//       
//    
//             //Write_Input(register_addr, 0xff);
//           
//             //register_value = Read_Reg_yj(register_addr);
//             //printk("0x55-Line = %d,register_addr = 0x%x: register_value = 0x%x\n",__LINE__,register_addr,register_value);
//            
//	     //Write_Input(register_addr, 0x7f);
//	  
//             //register_value = Read_Reg_yj(register_addr);
//             //printk("0xAA-Line = %d,register_addr = 0x%x: register_value = 0x%x\n",__LINE__,register_addr,register_value);
//       register_value = 0xff;
//       register_addr  = 0x40;     
//       Write_Input(register_addr, register_value);
//       msleep(80);
//       ret_value = Read_Output(0x29);
//       printk("0xff-Line = %d,register_addr = 0x%x: ret_value = 0x%x\n",__LINE__,register_addr,ret_value);
//
//       register_addr = 0x41;
//       Write_Input(register_addr, register_value);
//       msleep(80);
//       ret_value = Read_Output(0x2b);
//       printk("0xff-Line = %d,register_addr = 0x%x: ret_value = 0x%x\n",__LINE__,register_addr,ret_value);
//
//       register_addr = 0x42;
//       Write_Input(register_addr, register_value);
//       msleep(80);
//       ret_value = Read_Output(0x2d);
//       printk("0xff-Line = %d,register_addr = 0x%x: ret_value = 0x%x\n",__LINE__,register_addr,ret_value);	
//
//       register_value = 0x7f;
//       register_addr = 0x40;
...
//       msleep(80);
//       ret_value = Read_Output(0x29);
//       printk("0x7f-Line = %d,register_addr = 0x%x: ret_value = 0x%x\n",__LINE__,register_addr,ret_value);
//       register_addr = 0x41;
//       Write_Input(register_addr, register_value);
//       msleep(80);
//       ret_value = Read_Output(0x2b);
...
//input:xyz represent x y z axes
//0 for x
//1 for y
//2 for z
//output
//0 for cannot calibrate
//1 for can be calibrated
//input:xyz represent x y z axes
//0 for x
//1 for y
//2 for z
...
//0 for cannot calibrate
//1 for can be calibrated
static unsigned char Zero_Calibration(struct i2c_client *sc7660_i2c_client,unsigned char Axis, signed char rel)
{
	signed int    PosMaxOutput=0, NegMinOutput=0;
	signed int    PosOutput=0, NegOutput=0, MidOutput=0, Finaloutput=0, Newoutput=0;
	signed char   reg29=0;

	unsigned char PosOutputMisConfiguration = 0, NegOutputMisConfiguration = 0;
	unsigned char PosOutputOffConfiguration = 0, NegOutputOffConfiguration = 0;	
	unsigned char MidOutputMisConfiguration = 0, FinaloutputMisConfiguration = 0,NewoutputMisConfiguration = 0;
	unsigned char MidOutputOffConfiguration = 0, FinaloutputOffConfiguration = 0,NewoutputOffConfiguration = 0; 
	unsigned char count=0,cyclecount=0;
	unsigned char AddMis=0, AddOff=0, DataOutput=0;
	
	unsigned char (*MisDataSpaceConvert)(unsigned char continuous);
	unsigned char (*OffDataSpaceConvert)(unsigned char continuous);
	
  signed char   read_output = 0;
...
//step0=init
	if(Axis == 0)
	{
		...
		AddOff = 0x47;
		DataOutput = 0x29;
		Write_Input(AddOff, 0x3f);
		MisDataSpaceConvert    = reverse_MisDataSpaceConvert;
		OffDataSpaceConvert    = reverse_OffDataSpaceConvert;
	} 
	else if(Axis == 1)
	{
		AddMis = 0x41;
		...
		DataOutput = 0x2b;
		Write_Input(AddOff, 0x3f);
		MisDataSpaceConvert    = forword_MisDataSpaceConvert;
		OffDataSpaceConvert    = forword_OffDataSpaceConvert;
	} 
	else if(Axis == 2)
	{
		AddMis = 0x42;
...
//===========================================================================
//step1=can cali?
	Write_Input(AddMis, (*MisDataSpaceConvert)(255)); // x mis to Pos Max
	read_output  = Read_Output(DataOutput); 	        //read x output
	PosMaxOutput = (signed int)(read_output);
	if(PosMaxOutput < -25){
		printk("%sstep1 not finish at line: %d\n",__func__,__LINE__);
		return 0;// 
	}
	printk("line:%d Max:%d\n",__LINE__,read_output);
	if(abs(PosMaxOutput) < 3){
		printk("line:%d cali finish!Final=%d\n",__LINE__,PosMaxOutput);
		return 1;// if calibration ok,return
	}

	Write_Input(AddMis, (*MisDataSpaceConvert)(0));// x mis to Neg Min
	read_output  = Read_Output(DataOutput);        //read x output
	NegMinOutput = (signed int)(read_output);
	if(NegMinOutput > 25){
		printk("%sstep1 not finish at line: %d\n",__func__,__LINE__);
		return 0;// 
	}
	printk("line:%d Min:%d\n",__LINE__,read_output);
	if(abs(NegMinOutput) < 3){
		printk("line:%d cali finish!Final=%d\n",__LINE__,NegMinOutput);
		return 1;// if calibration ok,return
	}
	if(abs(PosMaxOutput - NegMinOutput) < 10){
		printk("step1 not finish at line: %d\n",__LINE__);
		return 0;// 
	}
//can cali?
//===========================================================================
...
//===========================================================================
//===========================================================================
//step2=cali mis for zero in 25 LSB		
	//First,miamatch calibrating with output <25LSB standard and dichotomy
	//assign max and min configuration to dichotomy compare array
	PosOutputMisConfiguration = 255;
	PosOutput = PosMaxOutput;
	NegOutputMisConfiguration = 0;
	NegOutput = NegMinOutput;
	cyclecount=0;
	while(0==IsNearZero(PosOutput, NegOutput, 25)){
		//error process
		cyclecount++;
		if(cyclecount>20){
			printk("step2 not finish at line: %d\n",__LINE__);
			break;
		}
		if((PosOutputMisConfiguration - NegOutputMisConfiguration)<2){
			printk("step2 not finish line: %d\n",__LINE__);
			break;// 
		}
		MidOutputMisConfiguration=(unsigned char)((PosOutputMisConfiguration + NegOutputMisConfiguration) / 2);
		Write_Input(AddMis, (*MisDataSpaceConvert)(MidOutputMisConfiguration));
		read_output = Read_Output(DataOutput);
		MidOutput   = (signed int)(read_output);
	
		printk("step2 Line %d:Pos=%d(0x%x),PosOut=%d,Mid=%d(0x%x),MidOut=%d,Neg=%d(0x%x),NegOut=%d\n",__LINE__,
				PosOutputMisConfiguration,(*MisDataSpaceConvert)(PosOutputMisConfiguration),PosOutput,
				MidOutputMisConfiguration,(*MisDataSpaceConvert)(MidOutputMisConfiguration),MidOutput,
				NegOutputMisConfiguration,(*MisDataSpaceConvert)(NegOutputMisConfiguration),NegOutput);
		if(MidOutput > 0)//if Mid mismatch output > 0,calibrat at Mid(new Pos) and last Neg
		{
			PosOutputMisConfiguration = MidOutputMisConfiguration;
			PosOutput = MidOutput;

		}
		else //if Mid mismatch output < 0,calibrat at last Pos and Mid(new Neg)
		{
			NegOutputMisConfiguration = MidOutputMisConfiguration;
			NegOutput = MidOutput;		
		}
	}
	//updata the finaloutput and its configuration
	if(1==IsNearZero(PosOutput, NegOutput, 25)){	
		Finaloutput = PosOutput;
		FinaloutputMisConfiguration = PosOutputMisConfiguration;		
	}			
	else if(2==IsNearZero(PosOutput, NegOutput, 25)){		
		Finaloutput = NegOutput;
		FinaloutputMisConfiguration = NegOutputMisConfiguration;		
	}			
	else{
		printk("step2 not finish line: %d\n",__LINE__);
		return 0;// 
	}		
	if(abs(Finaloutput) < 3) 
	{
		printk("line:%d cali finish!Final=%d\n",__LINE__,Finaloutput);
		return 1;// if calibration ok,return
	}
...
//===========================================================================
//===========================================================================
//step3=cali mis for rel in 25 LSB			
	PosOutputMisConfiguration = FinaloutputMisConfiguration + 20;
	Write_Input(AddMis,(*MisDataSpaceConvert)(PosOutputMisConfiguration));
	read_output = Read_Output(DataOutput);
	PosOutput   = (signed int)(read_output) - (signed int)rel;
	if(abs(PosOutput) < 3) {
		printk("line:%d cali finish!Final=%d\n",__LINE__,PosOutput);
		return 1;// if calibration ok,return
	}

	NegOutputMisConfiguration = FinaloutputMisConfiguration - 20;
	Write_Input(AddMis,(*MisDataSpaceConvert)(NegOutputMisConfiguration));
	read_output = Read_Output(DataOutput);
	NegOutput  = (signed int)(read_output) - (signed int)rel;
	if(abs(NegOutput) < 3){
		printk("line:%d cali finish!Final=%d\n",__LINE__,NegOutput);
		return 1;// if calibration ok,return
	}
	...
	while(0 == IsNearZero(PosOutput, NegOutput, 25)){
		...
		if((PosOutputMisConfiguration - NegOutputMisConfiguration) < 2){
			printk("step3 not finish line: %d\n",__LINE__);
			return 0;// 
		}
		MidOutputMisConfiguration = (unsigned char)((PosOutputMisConfiguration + NegOutputMisConfiguration) / 2);
		Write_Input(AddMis, (*MisDataSpaceConvert)(MidOutputMisConfiguration));
		read_output = Read_Output(DataOutput);
		MidOutput  = (signed int)(read_output) - (signed int)rel;
		printk("Step3 Line %d:Pos=%d(0x%x),PosOut=%d,Mid=%d(0x%x),MidOut=%d,Neg=%d(0x%x),NegOut=%d\n",__LINE__,
				PosOutputMisConfiguration,(*MisDataSpaceConvert)(PosOutputMisConfiguration),PosOutput,
				MidOutputMisConfiguration,(*MisDataSpaceConvert)(MidOutputMisConfiguration),MidOutput,
				NegOutputMisConfiguration,(*MisDataSpaceConvert)(NegOutputMisConfiguration),NegOutput);
		if(MidOutput > 0)//if Mid mismatch output > 0,calibrat at Mid(new Pos) and last Neg
	...
	if(abs(Finaloutput) < 3){
		reg29 = Read_Reg_yj(DataOutput);
		printk("line:%d cali finish!Final=%d,Reg=0x%x,RegDat=%d\n",__LINE__,Finaloutput,DataOutput,reg29);
		printk("line:%d cali finish!Final=%d\n",__LINE__,Finaloutput);
		return 1;// if calibration ok,return
	}		
//cali mis for zero in 25 LSB		
...
//step4=cali mis for rel and the most fit
	MidOutputMisConfiguration=FinaloutputMisConfiguration;	
	for(count = 0; count < 3; count++)
	{
		NewoutputMisConfiguration = MidOutputMisConfiguration + count - 1;
		Write_Input(AddMis, (*MisDataSpaceConvert)(NewoutputMisConfiguration));
		read_output =	Read_Output(DataOutput);
		Newoutput  = (signed int)(read_output)- (signed int)rel;
		if(abs(Newoutput) < abs(Finaloutput))
		{
			Finaloutput = Newoutput;
			FinaloutputMisConfiguration = NewoutputMisConfiguration;
		}	
...
//cali mis for rel and the most fit	
//===========================================================================
...
//===========================================================================
//===========================================================================
//step5=cali off for rel and the better fit
	Write_Input(AddOff, (*OffDataSpaceConvert)(0x7f)); // x off to Pos Max
	read_output  = Read_Output(DataOutput); 	  //read x output
	PosMaxOutput = (signed int)(read_output)- (signed int)rel;
	if(abs(PosMaxOutput) < 3){
		printk("line:%d cali finish!Final=%d\n",__LINE__,PosMaxOutput);
		return 1;// if calibration ok,return
	}
	Write_Input(AddOff, (*MisDataSpaceConvert)(0));// x off to Neg Min
	read_output  = Read_Output(DataOutput);//read x output
	NegMinOutput = (signed int)(read_output)- (signed int)rel;
	if(abs(NegMinOutput) < 3){
		printk("line:%d cali finish!Final=%d\n",__LINE__,NegMinOutput);
		return 1;// if calibration ok,return
	}
	PosOutputOffConfiguration = 0x7f;
	PosOutput = PosMaxOutput;
	NegOutputOffConfiguration = 0;
	NegOutput = NegMinOutput;
	cyclecount=0;
	while(0 == IsNearZero(PosOutput, NegOutput, 3)){
		//error process
		cyclecount++;
		if(cyclecount>20){
			printk("step5 not finish at line: %d\n",__LINE__);break;
		}
		if((PosOutputOffConfiguration - NegOutputOffConfiguration) < 2){
			printk("step5 not finish line: %d\n",__LINE__);break; 
		}
		MidOutputOffConfiguration = (unsigned char)((PosOutputOffConfiguration + NegOutputOffConfiguration) / 2);
		Write_Input(AddOff, (*OffDataSpaceConvert)(MidOutputOffConfiguration));
		read_output = Read_Output(DataOutput);
		MidOutput  = (signed int)(read_output)- (signed int)rel;
		printk("Step5 Line %d:Pos=%d(0x%x),PosOut=%d,Mid=%d(0x%x),MidOut=%d,Neg=%d(0x%x),NegOut=%d\n",__LINE__,
				PosOutputOffConfiguration,(*OffDataSpaceConvert)(PosOutputOffConfiguration),PosOutput,
				MidOutputOffConfiguration,(*OffDataSpaceConvert)(MidOutputOffConfiguration),MidOutput,
				NegOutputOffConfiguration,(*OffDataSpaceConvert)(NegOutputOffConfiguration),NegOutput);	
		if(MidOutput > 0)//if Mid mismatch output > 0,calibrat at Mid(new Pos) and last Neg
		{
			PosOutputOffConfiguration = MidOutputOffConfiguration;
			PosOutput = MidOutput;
		}
		else //if Mid mismatch output < 0,calibrat at last Pos and Mid(new Neg)
		{
			NegOutputOffConfiguration = MidOutputOffConfiguration;
			NegOutput = MidOutput;		
		}
	}
	//updata the finaloutput and its configuration
	if(1==IsNearZero(PosOutput, NegOutput, 3)){	
		Finaloutput = PosOutput;
		FinaloutputOffConfiguration = PosOutputOffConfiguration;		
	}			
	else if(2==IsNearZero(PosOutput, NegOutput, 3)){		
		Finaloutput = NegOutput;
		FinaloutputOffConfiguration = NegOutputOffConfiguration;		
	}			
	else{
		printk("step5 not finish line: %d\n",__LINE__);
		return 0;// 
...
//===========================================================================
//===========================================================================
//step6=cali off for rel and the best fit	
	for(count = 0; count < 5; count++){
		NewoutputOffConfiguration = MidOutputOffConfiguration + count - 2;
		Write_Input(AddOff, (*OffDataSpaceConvert)(NewoutputOffConfiguration));
		read_output = Read_Output(DataOutput);
		Newoutput  = (signed int)(read_output)- (signed int)rel;
		if(abs(Newoutput) < abs(Finaloutput)){
			Finaloutput = Newoutput;
			FinaloutputOffConfiguration = NewoutputOffConfiguration;
		}	
			
	}
	Write_Input(AddOff, (*OffDataSpaceConvert)(FinaloutputOffConfiguration));
	if(abs(Finaloutput) < 3){
		printk("line:%d cali finish!Final=%d\n",__LINE__,Finaloutput);
		return 1;// if calibration ok,return
	}
...
int auto_calibration_instant_yj(signed int x, signed int y, signed int z){
...
//	calidata.zfinalf=1;
	...
	if((calidata.xpoutput<-25)||(calidata.ypoutput<-25)||(calidata.zpoutput<-25)){
		printk("step2:=file=%s,line=%d\n",__FILE__,__LINE__);
                sensor_write_reg(sc7660_client_yj,0x13,0x01);//allen                                         
...
int auto_calibration_instant_yj_mtp(signed int x, signed int y, signed int z){
	return 1;
	/*
	...
	if(xfinalf && yfinalf && zfinalf)
	{
		...
		Write_Input(0x1e, 0x15);  //±£´æÐ£×¼¼Ä´æÆ÷µÄÐÞ¸Ä
		mdelay(300);
		reg_13 = sensor_read_reg(sc7660_client,0x13);
		printk("line %d  reg_13 = %x\n",__LINE__,reg_13);
		Write_Input(0x1e, 0);
...
static ssize_t auto_SC7660_3_axis_Calibration(int x, int y, int z)
{
	Write_Input(0x1e, 0x05);   //ÎªÐ£×¼¼Ä´æÆ÷µÄÐÞ¸Ä¿ª·ÅÈ¨ÏÞ
		unsigned temp[3];
    ...
    sensor_write_reg(sc7660_client_yj, 0x1e,0x05);
	mdelay(100);
	result = auto_calibration_instant_yj_mtp(0,0,-64);
	mdelay(10);
    if(result )
   	{
...
static ssize_t SC7660_calibration_run_store(struct device *dev,
        struct device_attribute *attr,
        const char *buf, size_t count)
{
    struct SC7660_acc acc;
  	long xyz[3]={0};
	int num = 0;
    int i,ret;
	...
	{	  
		...
		*buffer1 = SC7660_STATUS;
		ret = sensor_rx_data(sc7660_client_yj, buffer1,1);
		buffer1[0] &= 0x08;
		if(!buffer1[0])
		{
			i--;
		...
		*buffer2 = SC7660_XOUT_H;
		ret = sensor_rx_data(sc7660_client_yj, buffer2,1);
		if (ret < 0)
			return ret;
		xyz[0] += (((s16)((buffer2[0] << 8) + buffer1[0])) >>4);
		...
		*buffer1 = SC7660_YOUT_L;
		...
		if (ret < 0)
			return ret;
		xyz[1] += (((s16)((buffer2[0] << 8) + buffer1[0])) >>4);
		...
		*buffer1 = SC7660_ZOUT_L;
		...
		if (ret < 0)
			return ret;
		xyz[2] += (((s16)((buffer2[0] << 8) + buffer1[0])) >>4);
		msleep(1);
	}
	  acc.x=(s16)(xyz[0]/retry);
	  acc.y=(s16)(xyz[1]/retry);
	  acc.z=(s16)(xyz[2]/retry);
	if (acc.z > 0) {
      acc.z -= 1024;
    } 
	else {
...
{
    return sprintf(buf, "%d %d %d\n", offset_yj.x,offset_yj.y,offset_yj.z);
}
        ...
        const char *buf, size_t count)
{
    ...
    result = sensor_write_reg(sc7660_client_yj, address,value);
        ...
        struct device_attribute *attr, char *buf)
{
	...
	i = 0x0f;
    *buffer = i;	
    count = sensor_rx_data(sc7660_client_yj, &buffer[0], 1); 
    count += sprintf(buf, "0x%x: 0x%x\n", i, buffer[0]);
    for(i=0x10;i<0x5a;i++)
    {
		*buffer = i;
	
    	sensor_rx_data(sc7660_client_yj, &buffer[0], 1); 
...
static ssize_t SC7660_register13_reset(struct device *dev,
        struct device_attribute *attr,
        const char *buf, size_t count)
{
   // int address, value;
    ...
    result = sensor_write_reg(sc7660_client_yj, 0x1e,0x05);
	mdelay(100);
    result = sensor_write_reg(sc7660_client_yj, 0x13,0x00); 

    result = sensor_write_reg(sc7660_client_yj, 0x1e,0x15);
	mdelay(300);
...
static DEVICE_ATTR(calibration_reset, 0664,//S_IWUSR|S_IWGRP,
        NULL, SC7660_calibration_reset_store);
static DEVICE_ATTR(calibration_value,0664,//S_IRUGO|S_IWUSR|S_IWGRP,
        SC7660_calibration_value_show,
        SC7660_calibration_value_store);
static DEVICE_ATTR(value, 0664,//S_IRUGO,
        SC7660_value_show, NULL);
static DEVICE_ATTR(reg13_reset, 0664,//S_IRUGO,
        SC7660_register13_reset, NULL);
...
static int SC7660_load_user_calibration(struct i2c_client *client)
{
    s16 buffer[16];
    int ret = 0;
    s16 data[3];
    ...
    ret = SC7660_read_file(CFG_GSENSOR_CALIBFILE, buffer, sizeof(buffer));
    if (ret <= 0) {
...
//static int sc7660_acc_hw_init()
//{
//	int err = -1;       
//	
//	err = sensor_write_reg(sc7660_client_yj, SC7660_MODE, 0x77);
//	err = sensor_write_reg(sc7660_client_yj, SC7660_MODE3, 0x08);//open BDU
//	
//	//register_test();  
//
//	return err;    
...
//static int max_x=0, max_y=0, max_z=0, min_x=0, min_y=0, min_z=0;
//static long int sum_x = 0,sum_y = 0,sum_z = 0;
...
{	
	...
	if(result)
	{
    	...
    	calibrated = 0;//add by quanzhiyi
	sc7660_client_yj = client;
	sensor->status_cur = SENSOR_OFF;
	sensor->time_of_cali =0;//allen
    	offset_yj.x=offset_yj.y=offset_yj.z=0;
	  //sensor_write_reg(client, SC7660_BOOT, 0x80);
	//mdelay(20);
	result = sensor_write_reg(client, SC7660_MODE, 0x07);

 	
        sensor_write_reg(client, SC7660_MODE3, 0x88); //0x08);      //HR mode
//	result = sensor_write_reg(client, SC7660_MODE, 0x77);
	...
	reg_cali = sensor_read_reg(client,0x13);
...
static int gsensor_report_value(struct i2c_client *client, struct sensor_axis *axis)
{
	...
	{
		flag=0;
		input_report_abs(sensor->input_dev, ABS_X, -(axis->y)-1);//86v x
		input_report_abs(sensor->input_dev, ABS_Y, (axis->x)-1);//86v y
		input_report_abs(sensor->input_dev, ABS_Z, (axis->z)-1);
		input_sync(sensor->input_dev);
	}
	*/
	return 0;
...
static int sensor_report_value(struct i2c_client *client)
{
	...
	*/
	*buffer1 = 0xa8;
	ret = sensor_rx_data(sc7660_client_yj, buffer1,6);
	x = sensor_convert_data(sensor->client, buffer1[1], buffer1[0],0);
	y = sensor_convert_data(sensor->client, buffer1[3], buffer1[2],0);
	z = sensor_convert_data(sensor->client, buffer1[5], buffer1[4],0);
...
struct sensor_operate gsensor_sc7a20_ops = {
	.name				= "gs_sc7a20",
