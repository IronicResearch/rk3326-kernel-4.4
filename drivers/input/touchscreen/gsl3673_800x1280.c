...
//#include "rochkchip_gslX680_8inch_800x1280_tg806.h"
//#include "rockchip_gsl1680_7inch_800x480_HaoYuan_NEW.h"
//#include "rochkchip_gslX680_10inch_pb_800x1280.h"
#include "rockchip_863_gsl1680_8inch_800x1280_HaoYuan_NEW.h"
...
static int test_i2c(struct i2c_client *client)
{
	...
	if (ret < 0)
		rc--;
		printk("test_i2c read_buf:%d,rc:%d\n",read_buf,rc);
	msleep(2);
	...
	if (ret < 0)
		rc--;
		printk("1111test_i2c read_buf:%d,rc:%d\n",read_buf,rc);	
	return rc;
}
...
static void report_data(struct gsl_ts *ts, u16 x, u16 y, u8 pressure, u8 id)
{
	...
	if(y < MIDDLE_NUM)
	{
		y = SUB_NUM + y*(MIDDLE_NUM - SUB_NUM)/MIDDLE_NUM;
		//printk("000 x :%d,y:%d\n",x,y);
	}else{
		y = MIDDLE_NUM + (y - MIDDLE_NUM)*(MIDDLE_NUM - SUB_NUM)/MIDDLE_NUM;
		//printk("111 x :%d,y:%d\n",x,y);
	}
#endif	
  x = SCREEN_MAX_X - x;
