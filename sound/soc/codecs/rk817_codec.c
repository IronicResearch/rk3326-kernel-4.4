...
static const struct reg_default rk817_reg_defaults[] = {
	...
	{ RK817_CODEC_AHP_CFG0, 0x80 },
	{ RK817_CODEC_AHP_CFG1, 0x1f },
	{ RK817_CODEC_AHP_CP, 0x11 },
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if (rk817->capture_path != 0 && rk817->pdmdata_out_enable)
		snd_soc_update_bits(codec, RK817_CODEC_DI2S_CKM,
					PDM_EN_MASK, PDM_EN_ENABLE);
//yj add 
	if(snd_soc_read(codec, 0x3d)!=0x80)
		snd_soc_write(codec, 0x3d, 0x80);	
	if(snd_soc_read(codec, 0x3f)!=0x11)
		snd_soc_write(codec, 0x3f, 0x11);					
	return 0;
}
